...
struct TimeLineChartView: View {
    @Environment(HistoricalBarsViewModel.self) private var historicalBarsViewModel
    @State private var timeFrame: GlobalVars.Text.TimeLine = .oneHourPeriod
    ...
    var body: some View {
        ...
        .onAppear {
            Task { @MainActor
                let _ = await self.historicalBarsViewModel.getHistoricalBars()
            }
        }
    }
}
